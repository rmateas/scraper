...
export const getPagination = async (wsEndpoint, worker, proxy) => {
  ...
  try {
    ...
    ({
      allUrls: pagInfo.scrape.possibleInvUrls.allUrls,
      newUrls: pagInfo.scrape.possibleInvUrls.newUrls,
      usedUrls: pagInfo.scrape.possibleInvUrls.usedUrls,
      error: getPossibleInventoryUrlsError,
    } = await getPossibleInventoryUrls(page, worker));
    log({file, func, worker, level:'debug', message:'Possible Inv', obj:pagInfo.scrape.possibleInvUrls});
    ...
    let getBestPaginationOption = async (invUrlsArr) => {
      ...
      findPossiblePaginationFlow.length && pagInfo.scrape.paginationFlow.push(findPossiblePaginationFlow);
    ...
    let getNewInvUrl = pagInfo.scrape.possibleInvUrls.newUrls.length ? await getBestPaginationOption(pagInfo.scrape.possibleInvUrls.newUrls) : undefined;
    log({file, func, worker, level:'debug', message:'Possible NEW Inv', obj:getNewInvUrl});
    ...
    log({file, func, worker, level:'debug', message:'Possible USED Inv', obj:getUsedInvUrl});
